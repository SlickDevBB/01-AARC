...
export class AA_003_MK2_Grenade extends AarcActionBaseClass {
  // private variables
  ...
  public execute() {
    ...
    const svg = this.scene.add.image(this.actor.x, this.actor.y,'wearable-003');
    ...
    this.scene.tweens.add({
        ...
        duration: 750,
        onComplete: () => { 
          this.targets[0].adjustStat('HP_CURRENT', -damage);
          hitText(this.scene, this.actor, this.targets[0], Math.floor(damage).toString())
          svg.destroy();
          ...
          if (g) {
            ...
            else this.actor.adjustStat('ITEMS_RH', -1);  
          }
        },
    });
